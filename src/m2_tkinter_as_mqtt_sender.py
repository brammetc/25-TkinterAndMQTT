...
def send():
    ...
    mqtt_client.send_message("say_it", )
...
def main(mqtt_client):
    ...
    forward_button['command'] = lambda: mqtt_client.send_message('forward', [int(left_speed_entry.get()), int(right_speed_entry.get())])
    root.bind('<Up>', lambda event: print("Forward key"))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=3, column=0)
    left_button['command'] = lambda: mqtt_client.send_message('turn_left', [0, 400])
    root.bind('<Left>', lambda event: print("Left key"))

    stop_button = ttk.Button(main_frame, text="Stop")
    stop_button.grid(row=3, column=1)
    stop_button['command'] = lambda: mqtt_client.send_message('stop', [0, 0])
    root.bind('<space>', lambda event: print("Stop key"))

    right_button = ttk.Button(main_frame, text="Right")
    right_button.grid(row=3, column=2)
    right_button['command'] = lambda: mqtt_client.send_message('turn_right', [400, 0])
    root.bind('<Right>', lambda event: print("Right key"))

    back_button = ttk.Button(main_frame, text="Back")
    back_button.grid(row=4, column=1)
    back_button['command'] = lambda: mqtt_client.send_message('back', [-int(left_speed_entry.get()), -int(right_speed_entry.get())])
    root.bind('<Down>', lambda event: print("Back key"))

    up_button = ttk.Button(main_frame, text="Up")
    up_button.grid(row=5, column=0)
    up_button['command'] = lambda: mqtt_client.send_message('up', [180])
    root.bind('<u>', lambda event: print("Up key"))

    down_button = ttk.Button(main_frame, text="Down")
    down_button.grid(row=6, column=0)
    down_button['command'] = lambda: mqtt_client.send_message('down', [0])
